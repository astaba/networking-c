// ch02-socket-apis/time_server_ipv6.c

#include "chap02.h"
/**
 * @brief Simple cross-platform HTTP server demonstrating IPv6 socket binding.
 *
 * This example illustrates a minimal TCP server that listens on port 8080
 * using IPv6 addressing via the `getaddrinfo()` API. The server accepts one
 * incoming connection, logs the client address, and responds with the system's
 * current local time over a plain-text HTTP response.
 *
 * The code emphasizes the ease of switching to IPv6 support by adjusting
 * `hints.ai_family = AF_INET6` and shows how `[::1]` in the browser maps
 * cleanly to localhost 127.0.0.1 under this configuration.
 *
 * Socket setup and teardown are abstracted through macros for portability
 * across Windows (Winsock2) and Unix-like platforms.
 */
int main(void) {
#ifdef _WIN32
  WSADATA WSAData;
