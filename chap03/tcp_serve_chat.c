    ...
    for (i = 1; i <= max_socket; i++) {
      if (FD_ISSET(i, &readfds)) {
        if (i == socket_listen) { // Then accept a connection
          ...
          // INFO: This work by looping through all sockets in the master set.
          // For each socket j, we make sure it is neither the listening socket
          // nor the one sending data in the first place. Then send() is called
          // to echo the received data.
          SOCKET j;
          for (j = 1; j <= max_socket; ++j) {
            if (FD_ISSET(j, &master)) {
              if (j == socket_listen || j == i)
                ...
                send(j, recvbuf, bytes_received, 0);
            }
          }
        }
      } // end of if FD_ISSET
